...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        img = np.ascontiguousarray(img)
        if 'US' in self.img_files[index]:  # 自己加的
            img = np.expand_dims(img[0], axis=0)  # 自己加的
    ...
    def collate_fn(batch):
        ...
        img = list(img)  # 自己加的
        for i, im in enumerate(img):  # 自己加的
            if im.shape[0] == 1:  # 自己加的
                img[i] = torch.cat([im, im, im], dim=0)  # 自己加的
        img = tuple(img)  # 自己加的
...
if __name__ == '__main__':
    ...
    # train_loader, dataset = create_dataloader(path=r'../polyp_datasets\images\train', imgsz=640, batch_size=8,
    #                                           stride=32, single_cls=False, hyp=hyp, augment=True, cache='ram',
    #                                           rect=False, rank=-1, workers=0, image_weights=False, quad=False,
    #                                           prefix=colorstr('train: '), shuffle=True)
    task = 'test'
    dataloader = create_dataloader('../dataset_unpair/images/test/CDFI', 640, 32, 64, False, pad=0.5, rect=False,
                                   workers=0, prefix=colorstr(f'{task}: '))[0]
